// ═══════════════════════════════════════════════════════
// AUTH MODULE — Sign In / Sign Out / Profile Setup
// ═══════════════════════════════════════════════════════

let currentUser     = null;
let currentUserData = null;

// ── Sign In with Google ────────────────────────────────
async function signInWithGoogle() {
  try {
    const result = await auth.signInWithPopup(googleProvider);
    return result.user;
  } catch (err) {
    console.error('Sign-in error:', err);
    showToast('Sign-in failed: ' + err.message, 'error');
    throw err;
  }
}

// ── Sign Out ───────────────────────────────────────────
async function signOut() {
  try {
    await auth.signOut();
    currentUser     = null;
    currentUserData = null;
    showScreen('auth');
  } catch (err) {
    showToast('Sign-out error', 'error');
  }
}

// ── Check if user profile exists ──────────────────────
async function getUserProfile(uid) {
  try {
    const doc = await db.collection('users').doc(uid).get();
    return doc.exists ? doc.data() : null;
  } catch (err) {
    console.error('Get profile error:', err);
    return null;
  }
}

// ── Save new user profile ──────────────────────────────
async function saveUserProfile(uid, name, rollNumber, classSection, email, photoURL) {
  const isAdmin = ADMIN_EMAILS.includes(email.toLowerCase());
  const data = {
    uid,
    name,
    email,
    photoURL: photoURL || '',
    rollNumber,
    classSection,
    role: isAdmin ? 'teacher' : 'student',
    createdAt: firebase.firestore.FieldValue.serverTimestamp()
  };
  await db.collection('users').doc(uid).set(data);
  return data;
}

// ── Auth State Observer ────────────────────────────────
auth.onAuthStateChanged(async (user) => {
  if (user) {
    currentUser = user;
    const profile = await getUserProfile(user.uid);

    if (!profile) {
      // Brand new user → show profile setup
      hideLoading();
      showScreen('profile-setup');
      prefillSetupForm(user);
    } else {
      // Existing user
      currentUserData = profile;

      // If admin email but stored as student, upgrade
      if (ADMIN_EMAILS.includes(user.email.toLowerCase()) && profile.role !== 'teacher') {
        await db.collection('users').doc(user.uid).update({ role: 'teacher' });
        currentUserData.role = 'teacher';
      }

      hideLoading();
      showScreen('app');
      initApp(profile);
    }
  } else {
    hideLoading();
    showScreen('auth');
  }
});

// ── UI Helpers ─────────────────────────────────────────
function prefillSetupForm(user) {
  const nameInput = document.getElementById('setup-name');
  if (nameInput && user.displayName) nameInput.value = user.displayName;
}

// ── Profile Setup Submit ───────────────────────────────
document.getElementById('save-profile-btn').addEventListener('click', async () => {
  const name    = document.getElementById('setup-name').value.trim();
  const roll    = document.getElementById('setup-roll').value.trim();
  const cls     = document.getElementById('setup-class').value.trim();

  if (!name || !roll) {
    showToast('Name and Roll Number are required', 'warning');
    return;
  }

  const btn = document.getElementById('save-profile-btn');
  btn.textContent = 'Saving…';
  btn.disabled = true;

  try {
    const profile = await saveUserProfile(
      currentUser.uid, name, roll, cls,
      currentUser.email, currentUser.photoURL
    );
    currentUserData = profile;
    document.getElementById('profile-setup-modal').classList.add('hidden');
    showScreen('app');
    initApp(profile);
    showToast('Profile saved! Welcome to AashirTech 🎉', 'success');
  } catch (err) {
    showToast('Error saving profile: ' + err.message, 'error');
    btn.textContent = 'Save & Continue';
    btn.disabled = false;
  }
});

// ── Sign-in button ─────────────────────────────────────
document.getElementById('google-signin-btn').addEventListener('click', async () => {
  const btn = document.getElementById('google-signin-btn');
  btn.textContent = 'Signing in…';
  btn.disabled = true;
  try {
    await signInWithGoogle();
  } catch (e) {
    btn.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg> Continue with Google`;
    btn.disabled = false;
  }
});

// ── Sign-out button ────────────────────────────────────
document.getElementById('signout-btn').addEventListener('click', () => signOut());
